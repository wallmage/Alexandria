...
1 Write the subject file (subject, what the report must answer, reader), then init. Person subjects (incl. diaries, papers): add `--archetype person`. init prints the length target.
...
With ≥ 20 min remaining and the draft finished, in this order: `snapshot`, then light humanizing per `references/rewild/rewild/SKILL.md` (zh-CN `rewild-zh`, zh-HK `rewild-hk`) without touching quoted text, then `check`; the two reviews — `review start rewild` or `review start content` prints a skeleton into `reviews/`; the skeleton lists every field; fill only those; never read scripts/ or references/*.schema.json; `review finish` names anything still missing; a ledger patch with `brief`/`people`/`coverage`/`synthesis`. Their absence never blocks delivery.
